...
export const AuthGate: React.FC = () => {
  ...
  const copyDomainToClipboard = async () => {
    try {
      ...
      showToast(`Copy failed. Please copy manually: ${currentHostname}`, 'warning');
    }
  };
            ...
            <div className="bg-white text-slate-900 rounded-3xl shadow-2xl border border-emerald-100/50 overflow-hidden">
              ...
              <div className="p-5 sm:p-6 space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  {/* Domain Authorization Helper Toggle */}
                  <div className="flex items-center justify-between text-[11px] px-0.5 pt-0.5 text-slate-500">
                    <span className="text-[10px] text-slate-400 font-medium truncate max-w-[200px]">
                      Project: <code className="font-mono text-[9px] text-slate-600">{firebaseConfigInfo.projectId}</code>
                    </span>
                    <button
                      type="button"
                      onClick={() => setShowDomainHelp(!showDomainHelp)}
                      className="text-emerald-700 hover:text-emerald-900 font-semibold underline inline-flex items-center gap-1 text-[10px]"
                    >
                      <HelpCircle className="w-3 h-3 text-emerald-600" />
                      <span>{showDomainHelp ? 'Hide Domain Setup' : 'Authorize Domain for Google Sign-In'}</span>
                    </button>
                  </div>

                  {/* Domain Authorization Guide Card */}
                  {(unauthorizedDomain || showDomainHelp) && (
                    <div className="bg-amber-50/95 border border-amber-300 rounded-xl p-3.5 space-y-2.5 text-left transition-all shadow-xs">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <button
                          ...
                          className="text-amber-700 hover:text-amber-950 text-[10px] font-bold underline"
                        >
                          Dismiss
                        </button>
                      </div>
                      ...
                      <p className="text-[11px] text-amber-900 leading-relaxed">
                        To sign in with Google in this preview, Firebase requires the app’s domain to be added under <strong>Authorized domains</strong> in your Firebase Console.
                      </p>
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-1.5 bg-white border border-amber-300 rounded-lg p-1.5">
                          <code className="text-[11px] font-mono font-bold text-slate-800 flex-1 truncate px-1 select-all">
                            {currentHostname}
                          </code>
                          <button
                            type="button"
                            onClick={copyDomainToClipboard}
                            className="px-2.5 py-1 bg-amber-600 hover:bg-amber-700 text-white rounded text-[10px] font-bold inline-flex items-center gap-1 transition-colors shrink-0 shadow-2xs"
                          >
                            {copiedDomain ? (
                              <>
                      ...
                      <div className="space-y-1.5 pt-1">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-amber-800">
                          2. Quick steps in Firebase:
                        </span>
                        <ol className="text-[10px] text-amber-950 space-y-1 list-decimal list-inside font-medium bg-amber-100/70 p-2 rounded-lg">
                          <li>Click the button below to open Firebase Auth Settings.</li>
                          <li>Under <strong>Authorized domains</strong>, click <strong>Add domain</strong>.</li>
                          <li>Paste <code className="font-mono bg-white px-1 py-0.5 rounded text-[9px]">{currentHostname}</code> and click <strong>Add</strong>.</li>
                        </ol>
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="w-full py-2 px-3 bg-amber-700 hover:bg-amber-800 text-white rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 transition-all shadow-2xs"
                        >
                      ...
                      <div className="text-[10px] text-amber-900 pt-1.5 border-t border-amber-200">
                        ⚡ <strong>Instant bypass:</strong> Click <strong>Mr. Piseth Vanthan</strong> (<code className="font-semibold">vanthanbour@diu.edu.kh</code>) in the demo credentials below to log in immediately without Firebase domain setup!
                      </div>
                    </div>
                  )}
