...
import { LessonPlan, PlanAttachment, isAdminOrSuperAdmin } from '../types';
...
export const LessonPlanDetailModal: React.FC<LessonPlanDetailModalProps> = ({
  ...
  const isAcademicAuthority = Boolean(currentUser && (currentUser.role === 'admin' || currentUser.role === 'academic_officer' || isAdminOrSuperAdmin(currentUser)));
  const isPlanOwner = Boolean(currentUser && currentUser.role === 'teacher' && currentUser.id === plan.teacherId);
            ...
            <>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-emerald-50/60 rounded-2xl border border-emerald-200/80">
                <div className="flex items-center gap-3.5">
                  <UserAvatar
                    src={plan.teacherAvatar || allAccounts.find(a => (a.id && a.id === plan.teacherId) || (a.name && a.name.toLowerCase() === plan.teacherName.toLowerCase()))?.avatar}
                    name={plan.teacherName}
                    className="w-12 h-12 rounded-2xl ring-2 ring-emerald-500/30 shrink-0"
                  />
                  <div>
